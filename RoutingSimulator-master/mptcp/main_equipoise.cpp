...
int main(int argc, char **argv) {
  ...
  logfile.addLogger(sinkLogger);
	...
	for (int i=0;i<TCP_1;i++){
	  ...
	  ptcpSrc->setName("Tcp1"+ntoa(i)); logfile.writeName(*ptcpSrc);
	  ptcpSnk = new TcpSinkPeriodic(); ptcpSnk->setName("tcp1"+ntoa(i)); logfile.writeName(*ptcpSnk);
	  ...
	  routeout = new route_t(); routeout->push_back(pqueue);routeout->push_back(&queue1); routeout->push_back(&pipe1); routeout->push_back(ptcpSnk);
	...
	for (int i=0;i<TCP_2;i++){
	  ...
	  ptcpSnk = new TcpSinkPeriodic(); ptcpSnk->setName("tcp2"+ntoa(i)); logfile.writeName(*ptcpSnk);
	  ...
	  // tell it the route
	  routeout = new route_t(); routeout->push_back(pqueue); routeout->push_back(&queue2); routeout->push_back(&pipe2); routeout->push_back(ptcpSnk);
	...
	mtcp->setName("multipath");logfile.writeName(*mtcp);

	//MTCP flow 1
	tcpSrc = new TcpSrc(NULL,NULL,eventlist); tcpSrc->setName("Subflow1"); logfile.writeName(*tcpSrc);
	...
	//MTCP flow 2
	tcpSrc = new TcpSrc(NULL,NULL,eventlist); tcpSrc->setName("Subflow2"); logfile.writeName(*tcpSrc);
