...
public:
    ...
    enum TrafficEvent {
        PKT_ARRIVE = 0, PKT_DEPART = 1, PKT_CREATESEND = 2, PKT_DROP = 3, PKT_RCVDESTROY = 4
    };
...
public:
    enum QueueEvent {
        PKT_ENQUEUE = 0, PKT_DROP = 1, PKT_SERVICE = 2
    };
    enum QueueRecord {
        CUM_TRAFFIC = 0
    };
    enum QueueApprox {
        QUEUE_RANGE = 0, QUEUE_OVERFLOW = 1
    };
    enum EventType {
        QUEUE_EVENT = 0, QUEUE_RECORD = 4, QUEUE_APPROX = 5
    };
...
public:
    ...
    enum MultipathTcpEvent {
        CHANGE_A = 0, RTT_UPDATE = 1, WINDOW_UPDATE = 2, RATE = 3
    };
...
public:
    enum EventType {
        TCP_EVENT = 1, TCP_STATE = 2, TCP_RECORD = 6, TCP_SINK = 11
    };
    enum TcpEvent {
        TCP_RCV = 0, TCP_RCV_FR_END = 1, TCP_RCV_FR = 2, TCP_RCV_DUP_FR = 3,
        TCP_RCV_DUP = 4, TCP_RCV_3DUPNOFR = 5,
        TCP_RCV_DUP_FASTXMIT = 6, TCP_TIMEOUT = 7
    };
    enum TcpState {
        TCPSTATE_CNTRL = 0, TCPSTATE_SEQ = 1
    };
    enum TcpRecord {
        AVE_CWND = 0
    };
...
public:
    ...
    enum QcnEvent {
        QCN_SEND = 0, QCN_INC = 1, QCN_DEC = 2, QCN_INCD = 3, QCN_DECD = 4
    };
    enum QcnQueueEvent {
        QCN_FB = 0, QCN_NOFB = 1
    };
