...
public:
  ...
  CbrSrc(int src, EventList &eventlist, linkspeed_bps rate,simtime_picosec active=0,simtime_picosec idle=0);

 void connect(route_t& routeout, CbrSink& sink, simtime_picosec startTime);
 void installCbr(simtime_picosec startTime);
 void send_packet();
 void doNextEvent();
