...
int main(int argc, char **argv) {
  ...
  logfile.addLogger(sinkLogger);
	...
	for (int i=0;i<TCP_1;i++){
	  ...
	  tcpSrc->setName("Tcp1"+ntoa(i)); logfile.writeName(*tcpSrc);
	  tcpSnk = new TcpSinkPeriodic(); tcpSnk->setName("tcp1"+ntoa(i)); logfile.writeName(*tcpSnk);
	...
	for (int i=0;i<TCP_2;i++){
	  ...
	  tcpSnk = new TcpSinkPeriodic(); tcpSnk->setName("tcp2"+ntoa(i)); logfile.writeName(*tcpSnk);
	...
	//MTCP flow 1
	tcpSrc = new TcpSrcPeriodic(NULL,NULL,eventlist,0,0); tcpSrc->setName("Subflow1"); logfile.writeName(*tcpSrc);
	tcpSnk = new TcpSinkPeriodic(); tcpSnk->setName("Subflow1"); logfile.writeName(*tcpSnk);
	...
	sinkLogger.monitorSink(tcpSnk);
	//MTCP flow 2
	tcpSrc = new TcpSrcPeriodic(NULL,NULL,eventlist,0,0); tcpSrc->setName("Subflow2"); logfile.writeName(*tcpSrc);
	tcpSnk = new TcpSinkPeriodic(); tcpSnk->setName("Subflow2"); logfile.writeName(*tcpSnk);
	tcpRtxScanner.registerTcp(*tcpSrc);
